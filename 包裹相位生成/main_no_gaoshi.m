...
Hf = zeros(N);  % 初始化滤波器
u_range = max(1, u0-bw_u) : min(N, u0+bw_u);
v_range = max(1, v0-bw_v) : min(N, v0+bw_v);
Hf(v_range, u_range) = H(v_range, u_range);
% 显示原始和滤波信息
figure;
subplot(2,2,1);
...
%[phi_wrapped, U_rec] = fast_wff_phase_filter(phi_wrapped, 31, 31/6, 4 ,12);
%% =============================
